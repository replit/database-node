...
app.get('/db/:token/:key', async (req: Request, res: Response) => {
	...
	const request = await axios
		.get(join(dbUrl, encodeURIComponent(req.params.key || '')))
		.catch((err) => {
	...
	// Return the data
	return request.data
		? res.contentType('text').send((request.data || 'null').toString())
		: res.sendStatus(404);
});
...
app.delete('/db/:token/:key', async (req: Request, res: Response) => {
	...
	// Make a request to the DB
	const request = await axios
		.delete(join(dbUrl, encodeURIComponent(req.params.key || '')))
		.catch((err) => {
			// If the request returns 404, return 404
			if (err.response && err.response.status === 404) return 404;
			// Otherwise return 500
